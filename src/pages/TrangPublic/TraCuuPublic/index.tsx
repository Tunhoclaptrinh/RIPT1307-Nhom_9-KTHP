...
const TraCuuTuyenSinhPublic = () => {
	...
	const onFinish = async (values: any) => {
		...
		try {
			...
			const response = await axios.get(`${ipLocal}/users`, { params: queryParams });
			const users = response.data;
			const results = await Promise.all(
				users.map(async (user: any) => {
					const hoSoResponse = await axios.get(`${ipLocal}/hoSo?userId=${user.id}`);
					...
					const nguyenVongData = await Promise.all(
						nguyenVongIds.map((nvId: string) =>
							axios.get(`${ipLocal}/thongTinNguyenVong/${nvId}`).then((res) => res.data),
						),
					);
					const thongTinHocTapResponse = await axios.get(
						`${ipLocal}/thongTinHocTap?hoBaId=${hoSo.thongTinCaNhanId || ''}`,
					);
