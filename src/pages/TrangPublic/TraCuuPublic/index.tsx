			...
			const results = await Promise.all(
				matchedUsers.map(async (user: any) => {
					try {
						...
						// Sửa lỗi: nguyenVongDo là một chuỗi, không phải mảng
						const nguyenVongIds = hoSo.ketQua?.nguyenVongDo ? [hoSo.ketQua.nguyenVongDo] : [];
						...
						const nguyenVongData = await Promise.all(
							nguyenVongIds.map(async (nvId: string) => {
								try {
									...
									console.log(`Nguyện vọng ${nvId}:`, response.data); // Thêm log để kiểm tra
						...
						console.log('ValidNguyenVong:', validNguyenVong); // Thêm log để kiểm tra
						...
						try {
							const thongTinHocTapResponse = await axios.get(`${ipLocal}/thongTinHocTap?userId=${user.id}`);
						...
						if (Object.keys(thongTinHocTap).length === 0 && hoSo.thongTinHocTapId) {
							try {
								const thongTinHocTapResponse = await axios.get(`${ipLocal}/thongTinHocTap/${hoSo.thongTinHocTapId}`);
					...
					>
						<Form onFinish={onFinish} layout='vertical' form={form}>
							...
							<Row justify='center' style={{ marginTop: 20 }}>
								<Space size='middle'>
									<Button icon={<ClearOutlined />} onClick={handleReset} size='large' style={{ minWidth: 120 }}>
										Làm mới
									</Button>
									<Button
										icon={<SearchOutlined />}
