...
const DiemHocSinhPage = () => {
	...
	const renderDiemMonHoc = (diemMonHoc: DiemHocSinh.IDiemMonHoc[]) => {
		...
		return (
			<div>
				{diemMonHoc.slice(0, 3).map((diem, index) => (
					<div key={index} style={{ marginBottom: 4 }}>
						<Text strong>{diem.mon}</Text> - HK{diem.hocKy}: <Text type="success">{diem.diemTongKet}</Text>
					</div>
				))}
				{diemMonHoc.length > 3 && (
					<Text type="secondary">... và {diemMonHoc.length - 3} môn khác</Text>
				)}
			</div>
		);
	};
	...
	const columns: IColumn<DiemHocSinh.IRecord>[] = [
		...
		},
		{
			...
			width: 300,
			render: (diemMonHoc: DiemHocSinh.IDiemMonHoc[]) => renderDiemMonHoc(diemMonHoc),
		},
		{
