		...
		<div>
			...
			<Card title='Thông tin cá nhân' style={{ marginBottom: 16 }}>
				<Descriptions column={2} bordered>
					...
					<Descriptions.Item label='Ngày sinh'>
						{personalInfo.ngaySinh ? moment(personalInfo.ngaySinh).format('DD/MM/YYYY') : ''}
					</Descriptions.Item>
					<Descriptions.Item label='Email'>{personalInfo.email || ''}</Descriptions.Item>
					<Descriptions.Item label='Số điện thoại'>{personalInfo.soDT || ''}</Descriptions.Item>
					<Descriptions.Item label='Số CCCD'>{personalInfo.soCCCD || ''}</Descriptions.Item>
					<Descriptions.Item label='Giới tính'>{personalInfo.gioiTinh || ''}</Descriptions.Item>
					<Descriptions.Item label='Ngày cấp CCCD'>
						{personalInfo.ngayCap ? moment(personalInfo.ngayCap).format('DD/MM/YYYY') : ''}
					</Descriptions.Item>
					<Descriptions.Item label='Nơi cấp CCCD'>{personalInfo.noiCap || ''}</Descriptions.Item>
					...
					<Descriptions.Item label='Dân tộc'>{thongTinBoSung.danToc || 'Không'}</Descriptions.Item>
					<Descriptions.Item label='Quốc tịch'>{thongTinBoSung.quocTich || 'Không'}</Descriptions.Item>
					<Descriptions.Item label='Tôn giáo'>{thongTinBoSung.tonGiao || 'Không'}</Descriptions.Item>
					<Descriptions.Item label='Nơi sinh'>
						{thongTinBoSung.noiSinh?.trongNuoc
							? `${thongTinBoSung.noiSinh?.tinh_ThanhPho || ''}`
							: thongTinBoSung.noiSinh?.quocGia || 'Không'}
					</Descriptions.Item>
				</Descriptions>
			</Card>
			...
			{thongTinTHPT && (
				<Card title='Thông tin trường THPT' style={{ marginBottom: 16 }}>
					<Descriptions column={2} bordered>
						<Descriptions.Item label='Tên trường'>{thongTinTHPT.ten || ''}</Descriptions.Item>
						...
						<Descriptions.Item label='Năm tốt nghiệp'>{thongTinTHPT.namTotNghiep || ''}</Descriptions.Item>
					</Descriptions>
				</Card>
			)}
			...
			{showHocBa && hocBa.diemMonHoc && hocBa.diemMonHoc.length > 0 && (
				<Card title='Điểm học bạ đã nhập' style={{ marginBottom: 16 }}>
					<Descriptions column={1} bordered>
						...
						<Descriptions.Item label='Loại hạnh kiểm'>{hocBa.loaiHanhKiem || 'Không'}</Descriptions.Item>
						<Descriptions.Item label='Minh chứng'>{hocBa.minhChung || 'Không'}</Descriptions.Item>
					</Descriptions>
				</Card>
			)}
			...
			{diemDGTD.mon && diemDGTD.mon.length > 0 && (
				<Card title='Điểm đánh giá tư duy (DGTD)' style={{ marginBottom: 16 }}>
					<Descriptions column={1} bordered>
						...
						<Descriptions.Item label='Minh chứng'>{diemDGTD.minhChung || 'Không'}</Descriptions.Item>
					</Descriptions>
				</Card>
			)}
			...
			{diemDGNL.mon && diemDGNL.mon.length > 0 && (
				<Card title='Điểm đánh giá năng lực (DGNL)' style={{ marginBottom: 16 }}>
					<Descriptions column={1} bordered>
						...
						<Descriptions.Item label='Minh chứng'>{diemDGNL.minhChung || 'Không'}</Descriptions.Item>
					</Descriptions>
				</Card>
			)}
			...
			{giaiHSG.giaiHsgCap && (
				<Card title='Giải học sinh giỏi' style={{ marginBottom: 16 }}>
					<Descriptions column={1} bordered>
						<Descriptions.Item label='Cấp giải'>{giaiHSG.giaiHsgCap || 'Không'}</Descriptions.Item>
						<Descriptions.Item label='Môn'>{giaiHSG.mon || 'Không'}</Descriptions.Item>
						<Descriptions.Item label='Loại giải'>{giaiHSG.loaiGiai || 'Không'}</Descriptions.Item>
						<Descriptions.Item label='Năm'>{giaiHSG.nam || 'Không'}</Descriptions.Item>
						<Descriptions.Item label='Nơi cấp'>{giaiHSG.noiCap || 'Không'}</Descriptions.Item>
						<Descriptions.Item label='Minh chứng'>{giaiHSG.minhChung || 'Không'}</Descriptions.Item>
					</Descriptions>
				</Card>
			)}
					...
					{nguyenVong.map((nv: any, index: number) => (
						<Card key={index} type='inner' title={`Nguyện vọng ${index + 1}`} style={{ marginBottom: 8 }}>
							<Descriptions column={1} size='small' bordered>
								...
								<Descriptions.Item label='Cơ sở đào tạo'>{nv.coSoDaoTao || 'Không'}</Descriptions.Item>
